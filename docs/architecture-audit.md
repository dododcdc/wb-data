...
- [ ] **P1-3 offline 域体积失控**：后端 `OfflineFlowDocumentService` 已拆出 `FlowParameterBindingAssembler` / `FlowGraphDraftBuilder`（2026-09-13，约 977→634 行，方案见 [offline-document-service-split.md](offline-document-service-split.md)）。其余大文件仍待拆：`OfflineFlowYamlSupport` 813、`GitCommandService` 705、`KestraHttpClient` 633。前端 Top5 中 4 个在 views/offline（`TransferNodeDialog` 971、`OfflineWorkbench` 909、`useFlowEditingSession` 823、`FlowCanvas` 670；另有 `DataSourceForm` 748）。
