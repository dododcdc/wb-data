...
Brand voltage comes from the **cream + coral pairing** — coral (`{colors.primary}` — #D97757) is the signature Anthropic accent, used on every primary CTA, on the brand wordmark, and on full-bleed callout cards. The coral is warm, slightly muted, never cyan/blue — a deliberate counter-positioning against OpenAI's cool slate, Google's saturated blue, and Microsoft's corporate cyan.
...
- Coral primary CTA (`{colors.primary}` — #D97757). Used scarcely on individual buttons, generously on full-bleed coral callout cards.
...
### Brand & Accent
- **Coral / Primary** (`{colors.primary}` — #D97757): The signature Anthropic warm coral. Used on every primary CTA background, on full-bleed coral callout cards, on the brand wordmark accent. The most-recognized Anthropic color outside of the spike-mark logo.
- **Coral Active** (`{colors.primary-active}` — #b35a3b): The press / hover-darker variant.
...
### Font Family
The system runs **Copernicus** (or **Tiempos Headline** as substitute) as the slab-serif display face for headlines, and **StyreneB** (or **Inter** as substitute) as the humanist sans for body, navigation, and UI labels. **JetBrains Mono** handles code blocks. The fallback stack walks `Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif` for all text.
...
**`button-primary`** — The signature coral CTA. Background `{colors.primary}` (#D97757), text `{colors.on-primary}` (white), type `{typography.button}` (StyreneB 14px / 500), padding 12px × 20px, height 40px, rounded `{rounded.md}` (8px). Active state `button-primary-active` darkens to `{colors.primary-active}` (#b35a3b).
...
**`callout-card-coral`** — A full-bleed coral card carrying a major call-to-action. Background `{colors.primary}` (#D97757), text `{colors.on-primary}` (white), rounded `{rounded.lg}`, padding `{spacing.xxl}` (48px). The coral surface IS the voltage; the CTA inside uses an inverted button style (cream/canvas button on coral).
...
- Use Inter for all text. Headlines use weight 600 with tighter letter-spacing; body uses weight 400.
...
- Don't over-bold headlines. Weight 600 is sufficient for emphasis; weight 400 for body text.
...
5. Display headlines use Inter weight 600 with tighter tracking. Body stays Inter 400.
6. Cream + coral + dark navy is the trinity. Don't introduce a fourth surface tone (no purple cards, no green sections).
7. When in doubt about emphasis: bigger size before bolder weight.

## Known Gaps

- Inter is the project's primary font, available as a free web font. It is used for all text — display headlines and body.
